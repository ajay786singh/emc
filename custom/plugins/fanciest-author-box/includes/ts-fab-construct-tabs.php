...
function ts_fab_show_twitter( $context = '', $authorid = '' ) {
	...
	if ( get_user_meta( $author->ID, 'ts_fab_twitter', true ) ) {
		...
		if( isset( $result[0]->text ) && '' != $result[0]->text ) {
			...
			if( isset( $result[0]->user->description ) ) $description = $result[0]->user->description;
		} else {
	...
	if( get_user_meta( $author->ID, 'ts_fab_googleplus', true) ) {
		...
		<div class="ts-fab-tab" id="ts-fab-googleplus-' . $context . '">
			...
			<div class="ts-fab-text">
				...
				$ts_fab_googleplus .= '
				<g:plus href="https://plus.google.com/' . get_user_meta( $author->ID, 'ts_fab_googleplus', true ) . '" data-width="' . $width . '" data-layout="landscape" data-showcoverphoto="false"></g:plus>
			</div>
		</div>';
