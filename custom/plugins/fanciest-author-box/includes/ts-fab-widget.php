...
class ts_fab_widget extends WP_Widget {
	...
	function form( $instance ) {
		...
		<p>
			...
			// Get user count and display dropdown only if there's no more than 250 users	
			if ( count( $blogusers ) <= 250 ) {
			?>
			...
			<?php } else { 
			// If there's more than 250 users, show a number field, to avoid looping through all users
			?>
			<label for="<?php echo $this->get_field_id('author'); ?>">
				<?php _e( 'User ID:', 'ts-fab' ); ?>
			</label>
			<input type="number" class="widefat" id="<?php echo $this->get_field_id('author'); ?>" name="<?php echo $this->get_field_name('author'); ?>" value="<?php echo esc_attr($author); ?>" />
			<?php } ?>
		</p>
